...
<div class="board_toolbar">
    <ul class="m-0 p-0 list-board-subset mt-2">
        <li class="board-subset-item <?php if ($tab == 'table') echo 'active'; ?>" data-subset-title="table">
            ...
            <a class="text-dark w-100" href="<?= base_url() ?><?= ($project->type_id == 31) ? 'customtable' : 'table' ?>/view/<?= $parent_id ?>/<?= $project_id ?>">
        ...
        <?php foreach ($boards_subset as $board_subset) : ?>
            <li class="board-subset-item <?php if ($tab == $board_subset->title) echo 'active'; ?>" data-subset-title="<?= $board_subset->title ?>">
        ...
        <ul class="dropdown-menu menu-add-board-subset">
            ...
            <?php else : ?>
                <li data-project-id="<?= $project_id ?>" data-value="form" class="menu-board-subset-item px-2 dropdown-item" style="font-size: 14px;"> <span class="board-subset-icon"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span> <span> Form </span></li>
                <li data-project-id="<?= $project_id ?>" data-value="file" class="menu-board-subset-item px-2 dropdown-item" style="font-size: 14px;"> <span class="board-subset-icon"><i class="fa fa-picture-o" aria-hidden="true"></i></span> <span> File gallery </span></li>
                <li data-project-id="<?= $project_id ?>" data-value="calendar" class="menu-board-subset-item px-2 dropdown-item" style="font-size: 14px;"> <span class="board-subset-icon"><i class="fa fa-calendar" aria-hidden="true"></i></span> <span> Calendar </span></li>
                <li data-project-id="<?= $project_id ?>" data-value="kanban" class="menu-board-subset-item px-2 dropdown-item" style="font-size: 14px;"> <span class="board-subset-icon"><i class="fa fa-list-alt" aria-hidden="true"></i> </span> <span> Kanban </span></li>
                <li data-project-id="<?= $project_id ?>" data-value="gantt" class="menu-board-subset-item px-2 dropdown-item" style="font-size: 14px;"> <span class="board-subset-icon"><i class="fa fa-list-alt" aria-hidden="true"></i> </span> <span> Gantt </span></li>
            <?php endif; ?>
