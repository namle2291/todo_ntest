...
class User extends CI_Controller
{
    ...
    public function store()
    {
        ...
        $this->form_validation->set_rules('Username', 'Username', 'required|min_length[5]|max_length[50]|is_unique[users.username]|regex_match[/^[a-zA-Z0-9\s]+$/]');
        $this->form_validation->set_rules('Password', 'Password', 'required|min_length[5]|regex_match[/^[a-zA-Z0-9\s]+$/]');
        $this->form_validation->set_rules('ConfirmPassword', 'Confirm Password', 'required|matches[Password]|regex_match[/^[a-zA-Z0-9\s]+$/]');
